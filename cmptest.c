...
static void ScanArguments(int* argcPtr, char** argvPtr)
{
    int argc = *argcPtr;
    int i;
...
static int CmptestAppInit(interp)
Tcl_Interp* interp; /* Interpreter for application. */
{
    ...
    if (patchLevel)
    {
        ...
        if (!Tcl_SetVar(interp, "tcl_patchLevel", patchLevel, TCL_LEAVE_ERR_MSG))
        {
    ...
    Tcl_StaticPackage(interp, (char*)TbcloadGetPackageName(), Tbcload_Init, Tbcload_SafeInit);

    if (oldPatchLevel)
    {
        char* p = Tcl_SetVar(interp, "tcl_patchLevel", oldPatchLevel, TCL_LEAVE_ERR_MSG);
        Tcl_Free((char*)oldPatchLevel);
        if (!p)
        {
            return TCL_ERROR;
    ...
    Tcl_StaticPackage(interp, (char*)CompilerGetPackageName(), Compiler_Init, (Tcl_PackageInitProc*)NULL);

    if (Cmptest_Init(interp) == TCL_ERROR)
    {
        return TCL_ERROR;
    }
    Tcl_StaticPackage(interp, (char*)CmptestGetPackageName(), Cmptest_Init, (Tcl_PackageInitProc*)NULL);
